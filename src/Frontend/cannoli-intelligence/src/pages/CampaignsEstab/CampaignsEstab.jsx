...
export default function CampaignsEstab() {
  ...
  const chartBadge = useMemo(() => {
    ...
    return { labels, datasets: [{ label: "Qtd", data: values, backgroundColor: accent, borderRadius: 8 }] };
  }, [badgeData]);
        ...
        <div className="wrap">
            ...
            <div className="filters">
              <div className="field">
                ...
                <select value={campanhaId} onChange={e => setCampanhaId(e.target.value)}>
                  ...
                  {filtros.campanhas?.map((c, i) => <option key={i} value={c}>{c}</option>)}
                </select>
              </div>
              <div className="field">
          ...
          <section className="kpis">
            <div className="kpi">
              ...
              <div className="kpi_hint">Período: {mesInicio || "—"} — {mesFim || "—"}</div>
            </div>
            <div className="kpi">
              <div className="kpi_title">% Ativas</div>
              <div className="kpi_value">{kpis.ativasPct ?? 0}%</div>
              <div className="kpi_hint">Ativas: {kpis.ativas ?? 0} • Demais: {(kpis.total ?? 0) - (kpis.ativas ?? 0)}</div>
            </div>
            <div className="kpi">
              <div className="kpi_title">% Concluídas</div>
              <div className="kpi_value">{kpis.concluidasPct ?? 0}%</div>
              <div className="kpi_hint">Base: {kpis.total ?? 0} campanhas</div>
            </div>
            <div className="kpi">
              <div className="kpi_title">Novas no período</div>
              <div className="kpi_value">{kpis.novasPeriodo ?? 0}</div>
              <div className="kpi_hint">{mesInicio || mesFim ? "Com filtro de mês" : "Sem filtro de mês"}</div>
            </div>
          </section>
